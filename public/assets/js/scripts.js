      ...
      $wizard.each(function () {
        ...
        $self_id.steps({
          ...
          onInit: function (event, currentIndex) {
            // Tambahkan class untuk tombol Next dan Prev saat inisialisasi
            $wizard.find('.actions a[href="#next"]').addClass('btn btn-primary');
            $wizard.find('.actions a[href="#previous"]').addClass('btn btn-primary');
          },
          onStepChanging: function onStepChanging(event, currentIndex, newIndex) {
            // Allways allow previous action even if the current form is not valid!
            if (currentIndex > newIndex) {
              ...
              {
                $('#frmEditor').find('#btnSave').remove();
              }
              return true;
            }

            // $('#frmEditor').find('.actions a[href="#next"]').addClass('btn btn-primary');
            // $('#frmEditor').find('.actions a[href="#previous"]').addClass('btn btn-primary');
              ...
              if (currentIndex == 1)
                {
                  ...
                  $('#frmEditor').find('a[href="#finish"]').remove();
                  //append a submit type button
                  $('#frmEditor .actions li:last-child').append('<button type="button" id="btnSave" class="btn btn-primary">Save</button>');
                  $("#frmEditor .actions li:last-child").css("display","")
                }
                else {
                  $('#frmEditor').find('#btnSave').remove();
                }
